...
import com.roserequiem.app.domain.model.lyrics_providers.spotify.ServerTimeResponse
import com.roserequiem.app.domain.model.lyrics_providers.spotify.TrackSearchResult
import com.roserequiem.app.domain.model.lyrics_providers.spotify.WebPlayerTokenResponse
...
class SpotifyAPI {
    ...
    private suspend fun initializeTOTP() {
        ...
        totpMutex.withLock {
            if (totpGenerator != null) return@withLock
    ...
    suspend fun refreshToken(force: Boolean = false) {
        if (force || spotifyToken == "") {
            tokenMutex.withLock {
                if (!force && spotifyToken != "") return@withLock
